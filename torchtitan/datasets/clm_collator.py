...
class DataCollatorForCLM:
    ...
    def __call__(self, examples: List[Dict[str, List[np.ndarray]]]) -> Dict[str, Union[torch.Tensor, TensorPointer]]:
        ...
        if self.parallel_dims.pp_enabled:
            ...
            current_pp_rank = pp_mesh.get_rank()
        else:
            current_pp_rank = 0

        if self.parallel_dims.cp_enabled:
            cp_mesh = self.parallel_dims.world_mesh["cp"]
            cp_rank, cp_size = cp_mesh.get_rank(), cp_mesh.size()
        else:
            cp_rank, cp_size = 0, 1
        ...
        # TODO @nouamanetazi: Is it better to have examples as np.array or torch.Tensor?A
